    ...
    class PatternRecognizer
      ...
      def self.detect_candlestick_patterns(opens, highs, lows, closes)
        ...
        (2...closes.length).each do |i|
          ...
          # Engulfing patterns
          patterns << { type: :bullish_engulfing, index: i, strength: :medium } if bullish_engulfing?(first_candle, second_candle)

          patterns << { type: :bearish_engulfing, index: i, strength: :medium } if bearish_engulfing?(first_candle, second_candle)
      ...
      def self.detect_chart_patterns(highs, lows, closes)
        ...
        patterns << { type: :head_and_shoulders, strength: :strong, direction: :bearish } if head_and_shoulders?(highs, lows)
