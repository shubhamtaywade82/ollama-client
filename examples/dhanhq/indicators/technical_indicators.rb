    ...
    class TechnicalIndicators
      ...
      def self.support_resistance(highs, lows, closes, lookback = 20)
        ...
        (lookback...highs.length).each do |i|
          ...
          resistance_levels << { price: local_high, strength: window_highs.count(local_high) } if window_highs.count(local_high) >= 2

          # Support: price touched low multiple times
          support_levels << { price: local_low, strength: window_lows.count(local_low) } if window_lows.count(local_low) >= 2
        end
