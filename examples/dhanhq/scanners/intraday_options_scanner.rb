  ...
  module Scanners
    ...
    class IntradayOptionsScanner
      ...
      def scan_for_options_setups(underlying_symbol, exchange_segment: "IDX_I", min_score: 50, verbose: false)
        ...
        return error_result("Failed to analyze underlying: #{underlying_analysis[:error]}", verbose) if underlying_analysis[:error]
        ...
        return error_result("Failed to get expiry list: #{expiry_list_result[:error]}", verbose) if expiry_list_result[:error]
